...
IMG ?= csanchez/rollouts-plugin-metric-ai:latest
...
# Argo Rollouts is installed by default; skip with:
# - ARGO_ROLLOUTS_INSTALL_SKIP=true
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name kubebuilder-example-builder
	$(CONTAINER_TOOL) buildx use kubebuilder-example-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm kubebuilder-example-builder
	rm Dockerfile.cross
...
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/argo-rollouts && $(KUSTOMIZE) edit set image quay.io/argoproj/argo-rollouts=${IMG}
